...
export function MultiplayerApp({ initialCode = '', onExit }: { initialCode?: string; onExit: () => void }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = current.subscribe((message: ServerMessage) => {
      if (message.type === 'ERROR') { setJoining(false); setError(message.message); if (roomRef.current) current.send({ type: 'SYNC_STATE' }) }
      if (message.type === 'ROOM_STATE' || message.type === 'ROOM_PATCH') {
        ...
        version.current = update.version; clockOffset.current = update.clockOffsetMs; roomRef.current = update.snapshot; setRoom(update.snapshot); setJoining(false)
  ...
  const markWord = (wordId: string, status: 'correct' | 'passed') => {
    setRoom(previous => {
      ...
      const next = { ...previous, game: { ...previous.game, round: { ...previous.game.round, words: previous.game.round.words.map(word => word.id === wordId ? { ...word, status: word.status === status ? 'pending' as const : status } : word) } } }
  ...
  const toggleReady = () => {
    ...
    setRoom(previous => {
      ...
      const next = { ...previous, players: previous.players.map(player => player.id === playerId ? { ...player, ready } : player) }
      roomRef.current = next
      return next
    })
